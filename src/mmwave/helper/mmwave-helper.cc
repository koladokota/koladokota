...
void 
MmWaveHelper::SendSecondaryHandoverRequest (Ptr<Node> n, uint64_t imsi, uint16_t targetCellId, uint16_t oldCellId){
  EpcX2SapProvider::SecondaryHandoverParams params;
  params.imsi = imsi;
  params.targetCellId = targetCellId;
  params.oldCellId = oldCellId;
  n->GetObject<EpcX2> ()->GetEpcX2SapProvider()->SendMcHandoverRequest(params);
}
